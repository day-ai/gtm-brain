...
Pages carry **images** and **live page-to-page links**. Images upload in two steps against an *existing* page, and **the two calls are one operation — never stop after the first**: `get_page_image_upload_url` (jpeg/png/gif/webp only; rasterize vectors first) → upload the bytes → `attach_page_image`, which returns the embed snippet you place via `update_page`. Uploaded bytes without the `attach_page_image` call are invisible everywhere in the app — the image does not exist to the user until the attach completes, so an "upload succeeded" without an attach is a failure, not a partial success. Links come in two flavors: a page **chip** renders the target's live title and follows renames but shows no custom text; an inline **anchor** (`/page/{id}`) keeps your exact wording but doesn't follow renames. Chips for standalone references, anchors for inline prose. This is what lets an entire external doc corpus — folders, diagrams, cross-references — land in Day AI as a real knowledge base: the format contract and canonical apply algorithm live in `rollouts/preflight/README.md` (PAGES section), applied by `/sync-pages push context` or `/implement` preflight.
