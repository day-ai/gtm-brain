...
1. **Probe.** Confirm `create_page`/`update_page`/`read_page`/`create_or_update_folder` are on the connected tool list; probe for `get_page_image_upload_url` + `attach_page_image` (image upload is still rolling out). Missing image tools degrade (step 5); they never block the pages.
